...
fig <- plot_geo(leisure_data_state %>% filter(year == 2019), locationmode = 'USA-states')
fig <- fig %>% add_trace(
    z = ~avg_leisure_time, locations = ~state,
    color = ~avg_leisure_time, colors = 'Purples'
  )
fig <- fig %>% colorbar(title = "hours each day")
fig <- fig %>% layout(
...
fig <- plot_geo(leisure_data_state %>% filter(year == 2020), locationmode = 'USA-states')
fig <- fig %>% add_trace(
    z = ~avg_leisure_time, locations = ~state,
    color = ~avg_leisure_time, colors = 'Purples'
  )
...
Next, we want to fit a couple of models that we hypothesize may predict time spent on leisure activities, as well 1 model fitted using backwards selection.
...
We see a few outliers that will be noted and can be addressed in future analyses. Additionally, we would expect to see that the residuals are of random size across the outcome, and we can see that our residuals are extremely skewed, but for the purposes of this project, we will continue with our model comparisons.
