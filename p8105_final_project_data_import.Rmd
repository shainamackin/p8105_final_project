...
summary_household_category$race <- # Need to review race categorization
  as_factor(case_when(
    summary_household_category$race %in% c(4, 7, 10, 11, 13, 16, 17, 19, 22, 23) ~ "Asian",
    summary_household_category$race %in% c(1) ~ "White", 
    summary_household_category$race %in% c(2, 6, 12) ~ "Black",
    summary_household_category$race %in% c(5, 9, 18, 21) ~ "Hawaiian/Pacific Islander",
    TRUE ~ "unknown"
  ))
...
# model1 = lm(total_minutes ~ ., data = cps_summary_leisure_2019) %>% 
...
actsum_categorized_2020 = actsum_combined_categorized %>%
  filter(year == 2020)
# merge on case id, age, year, sex
# TRCHILDNUM, TRYHHCHILD, TELFS, TRETRNWA, TEMJOT, TRDPFTPT, TEHRUSLT, TRTEC, TRTHH, TESCHENR, TESCHLVL, TRSPRRES, TUDIARY, TRTEC, TRTHH, TESCHENR TESCHLVL, TESPEMPNOT, TRSPFTPTTRHOLiDAT, TUFINLWGT, TU06FWGT, TU20FWGT


resp_2020_tidied = resp_2020 %>% 
  select(TUCASEID, TULINENO, TERRP, TWSEX, TEAGE, TUYEAR, TUMONTH, TUDIARYDAY, TUDIARYDATE) %>%
  filter(TUYEAR == 2020)
```
...
>>>>>>> f1f26cc0df79f3f3d303899ddbe90e3d80cdb6d2
