...
Many individuals have used the specific data sets that we used for this project to do cool things! One example is a simulation built by a statistician at UCLA, who [simulated](https://flowingdata.com/2015/12/15/a-day-in-the-life-of-americans/) how 1000 Americans transition from one activity to another over the course of a day. We recommend you check it out.
<p/>
<p/>
# Initial Questions
...
**1.**  How do sociodemographic factors such as age, gender, and occupation affect time spent on work activities, leisure activities, and time spent with others? 

**2.** How does time use on different activity categories differ pre- and post-pandemic (2019 vs 2020)?

**3.** How do time use trends differ by geography and across the span of a decade?

**4.** How does time spent with others differ pre- and post- onset of COVID-19, and how do sociodempgraphic factors affect this? 

As the project evolved, however, so too did our initial questions:

+ Namely, in relation to Initial Question 4, we discovered that obtaining an estimate of time spent with others would require cleaning and merging two additional large datasets. Instead, we decided to modify our initial question and instead use the activity codes around socializing and communicating with others to get a sense of how this had changed from pre- to post-pandemic, and how this varied by age. 
...
For each of our visualizations and summary tables, we had to calculate average hours/minutes spent on activities using weights. The weights were created by ATUS to account for the fact that respondents were more likely to participate on weekends vs weekdays, and for the fact that some demographics were more represented than others (for instance, women were more likely to respond than men). To calculate average daily hours/minutes spent on an activity, we multiplied total time spent on the activity by the weight, and then summed all these products together and divided this by the sum of the weights. See a sample of this code below. 
...
We created multi-paneled plots to show how time spent on activities differed depending on age and sex. Below is the code we used for the age plot. 
...
The 3 main effect predictors in Model 2 are all significant (p-value < 0.05). Specifically, for every 1 year increase in age, we expect people to spend 0.059 hours more on leisure activities per day on average, adjusting for race and sex. We expect males to spend 0.785 hours more on leisure activities per day on average, adjusting for race and age. Finally, those of Hawaiian/Pacific Islanders descent are expected to spend 1.02 hours more per day on leisure activities when compared to people of mixed race, while those who are Asian spend the least time on leisure activities on average, adjusting for sex and age. 
